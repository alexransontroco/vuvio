...
def search_pixabay(
    ...
    for item in payload.get("hits", []):
        ...
        ranking = (
            math.log10(max(views, 1)) * 0.7
            + math.log10(max(likes, 1)) * 0.3
...
def detect_scene_times(video_path: Path) -> list[float]:
    """Détecte les changements de plan avec FFmpeg uniquement."""
...
def find_best_window(
    ...
    """Choisit une fenêtre intéressante à partir des scènes détectées."""
    del sample_every
    ...
    max_start = max(0.0, duration - actual_clip_duration)
    if not scene_times:
        return max_start / 2, actual_clip_duration, 0.0
    candidates = {0.0, max_start / 2, max_start}
    for timestamp in scene_times:
        candidates.add(max(0.0, min(timestamp - actual_clip_duration * 0.35, max_start)))
        candidates.add(max(0.0, min(timestamp - actual_clip_duration * 0.5, max_start)))
    best_start = max_start / 2
    best_score = -1.0
    for candidate in sorted(candidates):
        window_end = candidate + actual_clip_duration
        contained = [t for t in scene_times if candidate <= t <= window_end]
        scene_count = len(contained)
        if scene_count >= 2:
            gaps = [b-a for a,b in zip(contained, contained[1:])]
            regularity = 1.0 / (1.0 + (max(gaps)-min(gaps)))
        elif scene_count == 1:
            regularity = 0.25
        else:
            regularity = 0.0
        edge_penalty = (0.2 if candidate < 2.0 else 0.0) + (0.2 if window_end > duration-2.0 else 0.0)
        score = scene_count + regularity - edge_penalty
        if score > best_score:
            best_score = score
            best_start = candidate
    return round(best_start,3), actual_clip_duration, round(best_score,4)
