export default function BrandMark({ size = 34, showName = false, dark = false, withCircle = true, useLegacy = false }) {
  const textColor = dark ? '#062320' : '#ffffff';

  if (useLegacy) {
    const iconColor = dark ? '#062320' : null;
    ...
    if (!showName) {
      return <span className="brand-mark" aria-label="Vuvio">{legacyIcon}</span>;
    }
  ...
  const icon = (
    <img
      src="/assets/logo/vuvio-logo.png"
      alt="Vuvio"
      width={size}
      height={size}
      style={{ flexShrink: 0, display: 'block' }}
