 ...
 */
function validateImageMimeType(buffer) {
    if (buffer.length < 12) {
 ...
 */
async function generateThumbnailPlaceholder(buffer, _width, _height) {
 ...
 */
async function updateProductImageStatus(productId, status, urls, error) {
 ...
 */
export async function processProductImage(req, res) {
    // Enable CORS
    ...
    try {
        ...
        for (const size of sizes) {
            ...
            const thumbnailBuffer = await generateThumbnailPlaceholder(imageBuffer, size.width, size.height);
            // Upload to storage
        ...
        if (error instanceof ApiError) {
            ...
            const body = req.body;
            if (body.productId) {
                await updateProductImageStatus(body.productId, 'failed', undefined, error.message).catch(err => console.error('Failed to update product error status:', err));
            }
        }
        else {
            res.status(500).json({
