...
export async function turnCredentialsHandler(req, res) {
    try {
        ...
        const response = await fetch(`https://${appName}.metered.live/api/v1/turn/credentials?apiKey=${apiKey}`, { method: 'GET' });
        if (!response.ok) {
