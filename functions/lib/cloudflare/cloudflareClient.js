...
function normalizeLiveInput(result, customerCode) {
    ...
    const webRTCPlayback = result.webRTCPlayback;
    ...
    if (!webRTC)
        console.warn(`[CLOUDFLARE] webRTC field missing from Cloudflare response — constructed WHIP URL from streamKey: ${!!streamKey}`);
    return {
        liveInputId: uid,
        uid,
    ...
    return {
        async createLiveInput(input) {
            ...
            try {
                ...
                rawBody = await response.json().catch(() => ({}));
            }
            finally {
                clearTimeout(t);
            }
            const result = rawBody.result;
            const postRecording = result?.recording;
            ...
            if (uid) {
                try {
                    ...
                    const vRec = verify?.recording;
                    const vPlayback = verify?.playback;
        ...
        async listVideosByLiveInput(liveInputId) {
            ...
            return result.map((item) => ({
                ...
                playback: item.playback && typeof item.playback === 'object' ? item.playback : undefined,
