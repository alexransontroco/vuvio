...
export async function processHighlights() {
    try {
        ...
        for (const jobDoc of processingSnap.docs) {
            const job = jobDoc.data();
            const startedMs = job.startedAt?.toMillis?.() || 0;
...
async function processHighlightJob(liveId, jobId) {
    let job;
    try {
    ...
    try {
        ...
        const result = await generateHighlight(liveId, jobId, stream, job.useCreatorMarkers, creatorMarkers);
        // Update job to ready
