...
async function callCloudflareClipApi(recordingId, startSecond, durationSeconds) {
    const env = getCloudflareEnv();
    if (!env.accountId || !env.apiToken) {
        throw new ApiError('server_error', 'Cloudflare credentials not configured');
    }
    try {
        const response = await fetch(`https://api.cloudflare.com/client/v4/accounts/${env.accountId}/stream/${recordingId}/clip`, {
            method: 'POST',
            headers: {
...
function selectDefaultSegments(durationSeconds) {
...
export async function generateHighlight(liveId, jobId, stream, useCreatorMarkers, creatorMarkers) {
    try {
        ...
        for (let i = 0; i < segments.length; i++) {
            ...
            try {
                ...
                const clip = await callCloudflareClipApi(stream.cloudflareUid, segment.start, segment.duration);
                clips.push(clip);
