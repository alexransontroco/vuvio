...
function parseScheduledStartAt(value) {
    if (typeof value !== 'string')
        throw new ApiError('bad_request', 'scheduledStartAt is required');
    const date = new Date(value);
    if (!Number.isFinite(date.getTime()))
        throw new ApiError('bad_request', 'scheduledStartAt is invalid');
    if (date.getTime() < Date.now() + 5 * 60 * 1000) {
        throw new ApiError('bad_request', 'scheduledStartAt must be at least 5 minutes from now');
    }
    return Timestamp.fromDate(date);
}
export async function createScheduledStream(req, res) {
