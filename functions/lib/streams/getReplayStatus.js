...
export async function getReplayStatus(req, res, streamId) {
    ...
    if (!liveInputUid && !storedRecordingUid && !queryUid)
        throw new ApiError('bad_request', 'Missing Cloudflare liveInputId — was createLiveInput called?');
    ...
    if (!apiToken)
        throw new ApiError('server_error', 'Cloudflare not configured');
