...
const environments = new Set(['land', 'water', 'air', 'urban', 'other']);
...
export function stringField(input, key, options = {}) {
    const value = input[key];
    if (value === undefined || value === null) {
        if (options.required)
            throw new ApiError('bad_request', `${key} is required`);
        return undefined;
    }
    if (typeof value !== 'string')
        throw new ApiError('bad_request', `${key} must be a string`);
    const trimmed = value.trim();
    if (options.required && !trimmed)
        throw new ApiError('bad_request', `${key} is required`);
    if (options.max && trimmed.length > options.max)
        throw new ApiError('bad_request', `${key} is too long`);
    return trimmed;
}
export function stringArray(input, key, maxItems, maxLength = 48) {
    const value = input[key];
    if (value === undefined || value === null)
        return [];
    if (!Array.isArray(value))
        throw new ApiError('bad_request', `${key} must be an array`);
    if (value.length > maxItems)
        throw new ApiError('bad_request', `${key} has too many items`);
    return [...new Set(value.map((item) => {
            if (typeof item !== 'string')
                throw new ApiError('bad_request', `${key} must contain strings`);
            const trimmed = item.trim();
            if (!trimmed || trimmed.length > maxLength)
                throw new ApiError('bad_request', `${key} contains an invalid value`);
            return trimmed;
        }))];
}
...
export function parseApproximateLocation(input) {
...
export function safeMetadata(value) {
    ...
    if (json.length > 2048)
        throw new ApiError('bad_request', 'metadata is too large');
