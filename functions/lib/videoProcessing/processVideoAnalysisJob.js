...
async function runFfmpeg(args, logLabel) {
    if (!ffmpegPath)
        throw new ApiError('server_error', 'FFmpeg binary is not available');
...
function analysisPrompt(segments) {
    return [
...
function validateHighlights(input, durationSeconds) {
    const raw = Array.isArray(input?.highlights) ? input.highlights : [];
    const accepted = [];
...
export async function processVideoAnalysisJob(req, res, jobId) {
    const user = await authenticateUser(req);
    const jobRef = db.collection(JOB_COLLECTION).doc(jobId);
    const snap = await jobRef.get();
    if (!snap.exists)
        throw new ApiError('not_found', 'Video processing job not found');
    const job = snap.data() ?? {};
    if (job.userId !== user.uid)
        throw new ApiError('forbidden', 'This job belongs to another user');
    ...
    if (!sourceReplayUrl || !liveId)
        throw new ApiError('bad_request', 'Invalid job data');
    const openaiEnv = getOpenAIEnv();
    if (!openaiEnv.apiKey)
        throw new ApiError('server_error', 'OpenAI is not configured');
    ...
    try {
        ...
        const durationSeconds = safeNumber(transcription?.duration, transcriptDuration(transcript));
