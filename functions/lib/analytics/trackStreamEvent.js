...
import { allowedAnalyticsEvents } from '../types/analytics.js';
...
export async function trackStreamEvent(req, res, streamId) {
    const body = asRecord(req.body);
    const type = stringField(body, 'type', { required: true, max: 64 });
    if (!allowedAnalyticsEvents.includes(type))
        throw new ApiError('bad_request', 'Unknown analytics event type');
    const anonymousSessionId = stringField(body, 'anonymousSessionId', { max: 120 }) ?? null;
    const eventId = stringField(body, 'eventId', { max: 160 });
    const source = stringField(body, 'source', { max: 24 });
    if (source && !allowedSources.has(source))
        throw new ApiError('bad_request', 'Invalid event source');
    ...
    if (created)
        await updateStatsForEvent(streamId, type);
