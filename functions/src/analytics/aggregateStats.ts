...
interface CreatorStats {
  ...
  retention10sRate: number;
  retention30sRate: number;
  skipRate: number;
...
export async function aggregateStreamStats() {
  try {
    ...
    const streamsSnap = await db.collection(collections.streams as string).get();
...
export async function aggregateCategoryStats() {
  try {
    ...
    for (const streamDoc of streamsSnap.docs) {
      ...
      if (stream.status === 'live') stats.activeStreams += 1;
...
export async function aggregateUserAnalytics() {
  try {
    ...
    for (const userDoc of usersSnap.docs) {
      ...
      const affinityWeights: Record<string, number> = {
      ...
      for (const eventDoc of eventsSnap.docs) {
        ...
        if (event.category) {
          categoryAffinities.set(
            event.category,
            (categoryAffinities.get(event.category) ?? 0) + weight
          );
        }

        if (event.creatorId) {
          creatorAffinities.set(
            event.creatorId,
            (creatorAffinities.get(event.creatorId) ?? 0) + weight
          );
        }
        ...
        if (event.source) {
          sourceBreakdown.set(
            event.source,
            (sourceBreakdown.get(event.source) ?? 0) + 1
          );
        }
      }
