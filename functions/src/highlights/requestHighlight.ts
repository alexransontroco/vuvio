...
import type { HighlightRequest } from '../types/highlight.js';

const MIN_LIVE_DURATION = 60; // seconds

export async function requestHighlight(req: Request, res: Response, liveId: string) {
  const user = await authenticateUser(req);

  // Validate request body
  const body = req.body as HighlightRequest | null;
