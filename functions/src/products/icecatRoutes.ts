...
import {
  searchIcecatProducts,
  getIcecatProduct,
  mapIcecatToVuvioProduct,
} from './icecatService.js';
...
export async function searchIcecatHandler(req: Request, res: Response) {
  try {
...
export async function importIcecatHandler(req: Request, res: Response) {
  try {
    ...
    if (dupCheck.exists && dupCheck.productId) {
      ...
      logger.info(
        `[Icecat] Product already exists (${dupCheck.conflictField}): ${dupCheck.productId}`
      );
 ...
 */
async function searchLocalProducts(query: string, res: Response) {
  try {
...
async function enrichSearchResults(
  icecatResults: Array<Record<string, string>>,
  query: string
